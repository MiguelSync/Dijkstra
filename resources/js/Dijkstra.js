...
function exibirResultado(caminho, totalCusto) {
    const container = document.getElementById('caminho-container');
    ...
    if (!caminho || caminho.length === 0) {
        container.innerHTML = '<p class="text-center">Nenhum caminho encontrado</p>';
        return;
    }
    
    // Criar elementos para o caminho
    const row = document.createElement('div');
    row.className = 'row-caminho';
    
    for (let i = 0; i < caminho.length; i++) {
        // Adicionar vértice (cidade)
        const verticeContainer = document.createElement('div');
        verticeContainer.className = 'vertice-container';
        
        const vertice = document.createElement('div');
        vertice.className = 'vertice';
        vertice.textContent = caminho[i].nome;
        verticeContainer.appendChild(vertice);
        
        row.appendChild(verticeContainer);
        
        // Adicionar aresta (seta e informações) se não for o último vértice
        if (i < caminho.length - 1) {
            const arestaContainer = document.createElement('div');
            arestaContainer.className = 'aresta';
            
            // Linha horizontal com seta
            const linhaHorizontal = document.createElement('div');
            linhaHorizontal.className = 'linha-horizontal';
            arestaContainer.appendChild(linhaHorizontal);
            
            // Informações da aresta
            const infoAresta = document.createElement('div');
            infoAresta.className = 'info-aresta';
            infoAresta.innerHTML = `${caminho[i].aresta} | ${caminho[i].custo}`;
            arestaContainer.appendChild(infoAresta);
            
            row.appendChild(arestaContainer);
        }
    }
    ...
    // Adicionar resumo
    resumoContainer.innerHTML = `
        <h4 class="text-center mb-3">Resumo do Caminho</h4>
        <div class="resumo-item">Custo Total: <span class="resumo-destaque">${totalCusto}</span></div>
        <div class="resumo-item">Cidades Percorridas: <span class="resumo-destaque">${caminho.length}</span></div>
        <div class="resumo-item">Arestas Percorridas: <span class="resumo-destaque">${caminho.length - 1}</span></div>
    `;
}
